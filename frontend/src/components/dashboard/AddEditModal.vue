                ...
                <form v-if="activeTab === 'products'" @submit.prevent="handleSubmit">
                  <div class="grid grid-cols-1 gap-y-4 gap-x-4 sm:grid-cols-6">
                    ...
                    <div class="sm:col-span-3">
                      ...
                      <input type="number" step="0.01" id="price" v-model.number="productFormData.price" min="0.01"
                        class="p-2.5 h-10 mt-1.5 block w-full shadow-sm sm:text-sm border border-zinc-300 rounded-lg focus:ring-black focus:border-black transition-all"
                        required />
...
function validateProductForm() {
  ...
  productErrors.name = nameRegex.test(props.productFormData.name)
    ? ''
    : 'Name is required and must contain only letters.';

  productErrors.sku = props.productFormData.sku
    ? ''
    : 'SKU is required.';

  productErrors.category = props.productFormData.category
    ? ''
    : 'Category is required.';

  productErrors.stock = props.productFormData.stock >= 0
    ? ''
    : 'Stock must be a positive number.';

  productErrors.price = props.productFormData.price >= 0.01
    ? ''
...
function validateUserForm() {
  ...
  userErrors.name = nameRegex.test(props.userFormData.name)
    ? ''
    : 'Full name is required and must contain only letters.';

  userErrors.email = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(props.userFormData.email)
    ? ''
    : 'Enter a valid email.';

  userErrors.role = props.userFormData.role
    ? ''
    : 'Role is required.';

  userErrors.status = props.userFormData.status
    ? ''
    : 'Status is required.';

  userErrors.password = props.isEditing
    ? ''
    : (props.userFormData.password ? '' : 'Password is required.');
